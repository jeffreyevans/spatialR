...
I would note that MaxEnt is not a presence-only model per se but, generates a large number of regular samples to act as the null (background) with the know observations embedded in the background sample. This is not dissimilar to a PPM, (Renner & Warton 2013 demonstrated this equivalency) but, MaxEnt does not rely on an intensity function, rather solving a probability distribution via maximized entropy. 
...
```{.r .fold-show}
invisible(lapply(c("sf", "spatialEco", "terra", "raster", 
                   "randomForest","rfUtilities", "ggplot2",  
		               "ranger"), require, character.only=TRUE))
...
n = round( (nrow(den.pts) * p) / 2, 0)
 ( balance <- prop.table(table(ifelse(den.pts$y >= p, 1, 0))) )

cts <- table(ifelse(den.pts$y >= p, 1, 0))
suppressWarnings(
  msg <- warning("Not enough samples at this threshold,
  taking population"))
  if(cts[1]  < n) {
    cat(msg, "\n")
      ns <- cts[1]
      s1 <- den.pts[sample(which(den.pts$y < p),ns),]
  } else if(cts[1]  >= n){
    s1 <- den.pts[sample(which(den.pts$y < p),n),] 
  }    
 if(cts[2]  < n) {
    cat(msg, "\n")
      ns <- cts[2]
      s2 <- den.pts[sample(which(den.pts$y >= p),ns),]
  } else if(cts[2]  >= n){
    s2 <- den.pts[sample(which(den.pts$y >= p),n),] 
  }    
spp.den <- rbind(s1, s2)
...
par(mfrow=c(3,1))
  plot(spp.den.est, main="density estimate")
      plot(st_geometry(spp.pa), pch=20, cex=0.75, 
       col=my.col, add=TRUE)
  plot(crop(probs,spp.den.est), main="subset prob estimate")
      plot(st_geometry(spp.pa), pch=20, cex=0.75, 
       col=my.col, add=TRUE)      
  plot(probs, main="full prob estimate")
      plot(st_geometry(spp.pa), pch=20, cex=0.75, 
       col=my.col, add=TRUE)
