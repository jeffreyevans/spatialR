...
Here I proved an overview of methods, object types, classes, how to interact with objects (indexing), iteration, functions and spatial object classes. 
...
## Object definition and coersion
...
### Translate variance equation to code
...
#  We need, square of the summed deviations from the mean
...
### Write variance function 


```r
my.var <- function(x) 1/length(x) * sum((x - mean(x))^2)

# Test against the R var function 

var(x)
my.var(x)

# Why is it slightly different? population vs. 
# sample statistic!
```

## Write function to reclassify a vector
...
reclass <- function(x, p) ifelse(x <= p, 1, 0)
reclass(x, p = mean(x))

# We can then add an "if" statement to ensure that 
# the vector is numeric
reclass <- function(x, p) {
...
# Check if statement
reclass(as.character(x), p = mean(x) )
```


## Shannon's Entropy function (diversity)

$$H =\sum_{i=1}^{n} (p_{i} \log(p_{i}) )$$

### 1 - Proportion of species 

Proportion of species i at plot j – We can use apply to get row sums and then sweep to divide all values by row sums 
...
### 2- Natural log of proportions 

Natural log of proportion of species i at plot j – Here we will need a function that can be passed to apply
...
### 3 - Multiplication of proportions and log  

For multiplication of 1 and 2 – We can incorporate this step into the function developed for step #2 and then pass the function to apply
...
Now, put the pieces together and write a function for entropy
...
spp.df <- read.csv("http://s3.amazonaws.com/RTools/spp.plots.csv")
  spp <- meuse[sample(1:nrow(meuse), nrow(spp.df)),]
    spp <- cbind(spp, spp.df) 

# Shannon’s Diversity Index function
shannon <- function(x) {
  s <- function(x) { -1 * sum( x[x > 0] * log(x[x > 0]) ) }
  spp.sum <- apply(x, MARGIN = 1, sum)
  spp.prop <- sweep(x, MARGIN = 1, spp.sum, "/")
  apply(spp.prop, MARGIN = 1, FUN = s )
}

# Add results to data and plot
names(spp)
( diversity <- shannon( st_drop_geometry(spp[,13:ncol(spp)])) )
  spp$H <- diversity
...
ggplot() +
  ...
  labs(title = "Shannon Entropy (biodiversity)") +
  theme_void() +
  theme(legend.position = "bottom")
```

#  **Miscellaneous query and manipulation of sf objects**
...
# percent of soil class 1
length(meuse[meuse$soil == "1" ,]$soil) / length(meuse$soil)
length(meuse[meuse$soil != "1" ,]$soil) / length(meuse$soil)

    # or, more efficiently  
    prop.table(table(meuse$soil))

# mean of cadmium in soil class 1 
mean(meuse[meuse$soil == "1" ,]$cadmium)

# mean of cadmium for all soil classes
tapply(meuse$cadmium, meuse$soil, mean) 

# Conditional plot for soil class 1
plot(st_geometry(meuse[meuse$soil == "1" ,]), pch=20)

#### Split data into list object
soil <- list()
  for(i in unique(meuse$soil)) { soil[[i]] <- meuse[meuse$soil == i,] }

    # or, directly with lapply 
    soil <- lapply(unique(meuse$soil), function(i) {
                   meuse[meuse$soil == i,] })

# Plot each subset point list element
par(mfrow=c(2,2))
  lapply(soil, plot)

##### Split data into new sp subset objects in global namespace 
for(i in unique(meuse$soil)) { 
    assign(paste("soil", i, sep="."), meuse[meuse$soil == i,])
  }  

ls() # list objects in global namespace

# Plot soil subsets using "get" to call object from character variable
par(mfrow=c(2,2))
for(i in c("soil.1", "soil.2", "soil.3")) { plot(get(i), pch=20) }

# delete objects
remove(soil.1, soil.2, soil.3)
```
