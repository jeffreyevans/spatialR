...
An introduction to basic vector and raster analysis. We will also cover applying landscape metrics at the landscape and sample (plot) levels. 
...
Read "plots.shp" and "soil.shp" in as sf objects. Plot resulting objects. Hint; st_read, plot, st_geometry  
...
## Buffer points to 200m Hint; st_buffer 
...
Intersect (clip) soil with buffers and relate back to points. Hint; gIntersection and intersect) 
...
Calculate soil-type ("musym" attribute) area proportion for each plot. Hint; table, prop.table	
...
Point in polygon analysis to relate soil attributes to plots. Hint; point.in.poly  
...
Calculate summary statistics (global) for mean, sd and bakers choice. Hint; global, summary
...
Row standardize (x / max(x)) "ppt.jan" then, "ppt.jan" and "ppt.may" Hint; global, max and standard math operations
...
Calculate number of days with rain over median across all dates. Hint; app, median
...
Calculate focal mean of "ppt.jan" within an 11x11 window, plot results. Hint; focal, matrix 
...
Calculate percent of values <= global mean within an 11x11 window/ Hint; focal, global, matrix 
...
Reproject to UTM using "elev.tif" as reference raster. Hint; project, rast
...
Read the "plots.shp" points and "landcover.tif" raster. Hint; st_read, rast
...
Extract raster cell values (lc) for points (plots). Note; vector objects (ie., sf) must be terra vect class objects. Hint; extract and vect
...
Buffer the plots to 200m and then extract the "lc" raster cell values. What class is the object?  
...
## Aggregrate polygon raster values
...
Now write a function, to pass to tapply, that will calculate the proportion of value 23 and one for 41, 42 and 43. Add to data and play with plotting results. Hint; for, tapply, split, aggregrate  
...
Read "plots.shap" and "" Calculating landscape metrics for a entire landscape
...
## Reclassifing to forest/non-forest
...
Hint: classify, writing a function (ifelse) the calling app or using terra's native method for ifelse, ifel
...
Describe the landscape using landscape metrics and compare forest to forest cores. HINT: landscapemetrics, pland, contagion, proportion of like adjacency (many other available metrics, select those you think are most meaningful)

Hint; lsm_c_pland, lsm_l_pladj, lsm_l_contag  
...
Calculating landscape metrics for sample locations is a powerful way to compute covariates. Calculate landscape metrics for plots for both forest and cores. Hint; sample_lsm,
